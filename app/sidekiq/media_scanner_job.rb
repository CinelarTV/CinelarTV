...
class MediaScannerJob
  ...
  def perform
    ...
    sources_to_check = VideoSource.where.not(media_status: "checking")
                                  .where(
                                    "media_status = 'uncertain' OR last_checked_at IS NULL OR last_checked_at < ?",
                                    1.day.ago
                                  )
                                  .order(media_status: :desc, last_checked_at: :asc)
                                  .limit(batch_size)
