...
class WatchSession < ApplicationRecord
  ...
  scope :stale, ->(timeout = 7200) { active.where("updated_at < ?", timeout.seconds.ago) }
  ...
  def watch_percentage
    ...
    [(duration_watched / total_duration * 100).round(1), 100.0].min
  end
  ...
  def self.end_stale_sessions!(timeout_seconds = 7200)
    stale(timeout_seconds).find_each(&:end_session!)
  end
end
